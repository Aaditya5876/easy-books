...
function ClassDialog({ open, onClose, initial, employees, companyId }) {
  ...
  const save = useMutation({
    ...
    onError: (err) => toast.error(err?.response?.data?.message || 'Failed to save class'),
  });
      ...
      <DialogContent className="max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          <div className="space-y-1.5">
            ...
            <Input placeholder="e.g. Grade 5, Class 10" value={form.name} onChange={e => set('name', e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label>Section <span className="text-muted-foreground">(Optional)</span></Label>
            <Input placeholder="e.g. A, B, Science" value={form.section} onChange={e => set('section', e.target.value)} />
          </div>
          <div className="space-y-1.5">
...
export default function Classes() {
  ...
  const remove = useMutation({
    ...
    onError: (err) => toast.error(err?.response?.data?.message || 'Cannot delete — students are enrolled in this class'),
  });
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <Button className="mt-4" size="sm" onClick={() => setDialog({ mode: 'add' })}>Add First Class</Button>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 border-b border-border">
                <tr>
                  ...
                  <th className="text-left px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">Section</th>
                  <th className="text-left px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">Class Teacher</th>
                  <th className="text-right px-5 py-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">Students</th>
                  ...
                  <tr key={cls.id} className="hover:bg-muted/20">
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        <button
                          onClick={() => {
                            if (confirm(`Delete ${cls.name}${cls.section ? ` (${cls.section})` : ''}?`)) remove.mutate(cls.id);
                          }}
