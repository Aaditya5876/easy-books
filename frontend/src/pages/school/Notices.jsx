...
function NoticeDialog({ open, onClose, notice }) {
  ...
  async function generateWithAI() {
    if (!form.title.trim()) { toast.error('Enter a topic/title first so AI knows what to write'); return; }
    setAiLoading(true);
    try {
      ...
      toast.error(e?.response?.data?.message || 'AI generation failed — check GEMINI_API_KEY');
    } finally {
      setAiLoading(false);
    }
  }
            ...
            <div key={n.id} className={`rounded-lg border p-4 bg-card ${isExpired(n) ? 'opacity-60' : ''}`}>
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex gap-1 shrink-0">
                  ...
                  <Button size="icon" variant="ghost" title="Broadcast SMS to all guardians" onClick={async () => {
                    try {
